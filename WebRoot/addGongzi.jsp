			...
			<tr>
				<td class="border_left border_right border_bottom" height="400"
					...
					<div id="formwrapper">
						...
						<form action="<%=path%>/GongziServlet?method=insert"
							...
							<fieldset>
								...
								<div>
									...
									<%
										...
										List<HashMap> list1 = dao1.select("select * from t_user  ");
									%>
									<select name="userid">
										<%
											for (HashMap map1 : list1) {
										%>
										<option value="<%=map1.get("id")%>">
											<%=map1.get("bumen")%>  &nbsp;&nbsp;     <%=map1.get("name")%>
										</option>
								...
								<div>
									...
									<input type="text" class="Wdate" style="width: 160px"
										onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" id="riqi"
										name="riqi" value="<%=Utils.getCurDate() + ""%>" />
									<br />
								</div>
