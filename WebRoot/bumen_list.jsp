...
String biaoqian=request.getParameter("biaoqian");
						 ...
						 <td align="center"><%=alRow.get(1)%></td> 
								<td align="center" width="25%">
									  <a href="<%=path%>/editBumen.jsp?id=<%=alRow.get(0)%>"
										class="admin">编辑</a> &nbsp;
									<a href="<%=path%>/BumenServlet?method=delete&id=<%=alRow.get(0)%>"
										class="admin">删除</a>
								</td>
							</tr>
							<%
							...
							<tr class="tabletop">
								<td height="25" align="center">
									<a href="<%=path%>/addBumen.jsp">添加部门</a>
								</td>
								<td colspan="6" align="center">
									...
									for (int i = 0; i < Math.ceil(products.size()
											...
											if (pages == (i + 1)) {
								%>
									<a href="<%=path%>/bumen_list.jsp?pages=<%=i + 1%>"><font
										color="red"><%=i + 1%></font> </a>&nbsp;
									<%
										} else {
									%>
									<a href="<%=path%>/bumen_list.jsp?pages=<%=i + 1%>"><%=i + 1%></a>&nbsp;
