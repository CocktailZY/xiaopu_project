...
<body>
	...
	<div id="header">
		<h1>
			<a href="dashboard.html">商品类型管理</a>
		</h1>
	</div>
	<!--close-Header-part-->
		...
		<div id="content-header">
			<div id="breadcrumb">
				<a href="<%=basePath%>view/admin/admin.jsp" title="Go to Home"
					...
					class="current">商品类型管理</a>
			</div>
		</div>
		<div class="container-fluid">
			<div class="row-fluid">
				<div class="span12">
					<button class="btn" onclick="window.location.href='<%=basePath%>view/admin/shop/addproductType.jsp'">增加类型</button>
					<button class="btn" onclick="window.location.href='<%=basePath%>productType/list.do'">所有类型</button>
					<button class="btn" onclick="window.location.href='<%=basePath%>productType/list.do?type=1'">一级类型</button>
					<button class="btn" onclick="window.location.href='<%=basePath%>productType/list.do?type=2'">二级类型</button>
					<button class="btn" onclick="window.location.href='<%=basePath%>productType/list.do?type=3'">三级类型</button>
								...
								<tbody>
									<c:forEach items="${list }" var="item">
										<tr class="gradeX">
											...
											<td class="center">
												<button class="btn" data-reveal-id="myModal${item.ptId }" data-animation="fade" onclick="javascript:set_fpower('${item.parentid}','${item.type}')">修改</button>
										...
										<div id="myModal${item.ptId }" class="reveal-modal">
											<h3>修改类型</h3>
											<form action="<%=basePath%>productType/update.do" method="post">
												类型名称:<input type="text" name="ptName" value="${item.ptName}"><br>
												<input type="hidden" name="ptId" value="${item.ptId}">
...
<%-- 	<script src="<%=basePath%>js/select2.min.js"></script> --%>
	...
	<script type="text/javascript">
			function delete_shop(pid,fid) {
				...
				function()
				{
					window.location.href="<%=basePath%>productType/delete.do?ptId="+pid+"&parentid="+fid;
				}
			...
			function serch(id){
				if($("#childlist").css("display")=='none'){
					...
					async:true,
					dataType:"json",
					url:"<%=basePath%>productType/childTypelist.do",
					data: {
						ptId:id
					},
			...
			function set_fpower(pid,typeid){
				var fpowerid = pid;
				if(typeid != 1){
					$("select[name=parentid]").removeClass("yincang");
					$.ajax({
						type:"post",
						async:true,
						dataType:"json",
						url:"<%=basePath%>productType/listFType.do",
						data:{
							type:typeid
						},
						success:function(data){
							var json = eval(data);
							var row2 = $("select[name=parentid]");
							row2.empty();
							var td = "";
							for(var i=0;i<json.length;i++){
								if(json[i].ptId != null){
					...
					$("select[name=parentid]").addClass("yincang");
				}				
				}
