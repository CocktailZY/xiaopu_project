...
<div id="content">
  ...
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="span12">
      	${role.roName }
      	<button class="btn btn-warn" onclick="window.location.href='<%=basePath%>rolePower/addPower.do?roId=${role.roId }'">添加权限</button>
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            ...
            <table class="table table-bordered data-table">
              ...
              <tbody>
              	<c:forEach items="${list }" var="item"><!-- 该角色的所有权限,item是权限对象 -->
	                <tr class="gradeX">
	                  	<td>${item.poId }</td>
	                  	<td>${item.poName }</td>
	                  	<td class="center">
	                  		<button class="btn" onclick="window.location.href='<%=basePath%>rolePower/childPower.do?poId=${item.poId }&roId=${role.roId }'">修改</button>
	                  		<button class="btn btn-danger" onclick="delete_shop('${item.poId }','${role.roId }')">删除</button>
	                  	</td>
	                </tr>
               	</c:forEach>
