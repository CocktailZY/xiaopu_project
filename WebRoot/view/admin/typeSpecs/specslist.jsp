  ...
  <ul>
	...
	<div id="content">
		...
		<div class="container-fluid">
			<div class="row-fluid">
				<div class="span12">
					<button class="btn" onclick="window.location.href='<%=basePath%>typeSpecs/addSpecs.do'">增加规格</button>
								...
								<tbody>
									<c:forEach items="${list }" var="item">
										<tr class="gradeX">
											...
											<td class="center">
												<button class="btn" data-reveal-id="myModal${item.tsId }" data-animation="fade" onclick="javascript:set_fpower('${item.tsProducttypeid }')">修改</button>
												<button class="btn" onclick="window.location.href='<%=basePath%>typeSpecs/findSpecs.do?tsId=${item.tsId }'">编辑</button>
												<button class="btn btn-danger" onclick="delete_shop('${item.tsId }','${item.tsProducttypeid }')">删除</button>
											</td>
										</tr>
										
										<div id="myModal${item.tsId }" class="reveal-modal">
											<h3>修改规格</h3>
											<form action="<%=basePath%>typeSpecs/update.do" method="post">
												规格名称:<input type="text" name="tsTitle" value="${item.tsTitle}"><br>
...
<%-- 	<script src="<%=basePath%>js/select2.min.js"></script> --%>
	...
	<script type="text/javascript">
			function delete_shop(pid,fid) {
				...
				function()
				{
					window.location.href="<%=basePath%>typeSpecs/delete.do?tsId="+pid+"&tsProducttypeid="+fid;
				}
