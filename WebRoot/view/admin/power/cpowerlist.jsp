							...
							<table class="table table-bordered data-table">
								...
								<tbody>
									<c:forEach items="${clist }" var="citem">
										<tr class="gradeX">
											<td>${citem.poId }</td>
											<td><a href="javascript:serch('${citem.parentid }')">
											
											<c:forEach items="${plist }" var="pitem">
												<c:if test="${citem.parentid == pitem.poId}">
													${pitem.poName }
												</c:if>
											</c:forEach>
											...
											<td>${citem.poName}</td>
											<td class="center">
												<button class="btn" data-reveal-id="myModal${citem.poId }" data-animation="fade" onclick="javascript:set_fpower('${citem.parentid}')">修改</button>
												<button class="btn btn-danger" onclick="delete_shop('${citem.poId }','${citem.parentid}')">删除</button>
											</td>
										</tr>
										...
										<div id="myModal${citem.poId }" class="reveal-modal">
											<h3>修改权限</h3>
											<form action="<%=basePath%>power/update.do" method="post">
												权限名称:<input type="text" name="poName" value="${citem.poName}"><br>
												父权限名称:
													<select name="parentid">
														<option value="0" selected="selected">请选择父权限</option>
													</select>
												<input type="hidden" name="poId" value="${citem.poId}"><br>
												URL:<input type="text" name="url" value="${citem.url}"><br>
				...
				var fpowerid = pid;
					$.ajax({
						...
						success:function(data){
							...
							for(var i=0;i<json.length;i++){
								if(json[i].poId != null){
									if(json[i].poId == fpowerid){
										td = $("<option selected="+"selected"+">").text(json[i].poName).val(json[i].poId);
									}else{
										td = $("<option>").text(json[i].poName).val(json[i].poId);
									}
									row2.append(td);
								}
							}
