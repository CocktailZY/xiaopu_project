...
<script type="text/javascript" src="<%=basePath %>js/jquery.jscrollpane.min.js"></script>
				...
				<div class="container">
					<div class="header-search">
						<form action="<%=basePath %>product/getAllProductByPage.do" method="post">
							<input type="search" name="Search" placeholder="搜索商品" required="" value="${Search }">
	...
	<div class="products">
		<div class="container">
			<div class="col-md-9 product-w3ls-right">
				...
				<form action="product/typeSelect.do" method="post" id="findFrom">
				<input type="hidden" id="pageNum" name="pageNum">
				<input type="hidden" id="idd" name="id" value="${id }">
				<c:forEach var="g" begin="${begin }" end="${end }">
					<div class="col-md-3 product-grids">
						<div class="agile-products">
							<div class="new-tag">
							<c:if test="${generalList[g].product.pNew==1 }">
								<h6>
									NEW<br>
								</h6>
							</c:if>	
							<c:if test="${generalList[g].product.pNew==0 }">
								<h6>
									<span id="oldDiscount"><fmt:formatNumber type="number" value="${generalList[g].product.pDiscount * 100 }" /></span><br>折
								</h6>
							</c:if>
							</div>
							<a href="single.html"><img src="<%=basePath %>fileUpload/${generalList[g].images.imAddress}" class="img-responsive" alt="img"></a>
							<div class="agile-product-text">
								<h5>
									<a href="view/shop/cart.jsp">${generalList[g].product.pName }</a>
								</h5>
								<c:if test="${generalList[g].product.pNew==0 }">
								<h6>
								
									<del id="oldPrice">$${generalList[g].productSpecs.psPrice }</del><span>$<fmt:formatNumber type="number" value="${generalList[g].product.pDiscount*generalList[g].productSpecs.psPrice }" maxFractionDigits="2" /></span>
								</h6>
								</c:if>
								<c:if test="${generalList[g].product.pNew==1 }">
								<h6>
									$${generalList[g].productSpecs.psPrice }
								</h6>
								</c:if>
								<form action="#" method="post">
									...
									<input type="hidden" name="w3ls_item" value="${generalList[g].product.pName }" />
									<c:if test="${generalList[g].product.pNew==0 }"> 
									<input type="hidden" name="amount" value="${generalList[g].product.pDiscount*generalList[g].productSpecs.psPrice }" />
									</c:if>
									<c:if test="${generalList[g].product.pNew==1 }">
									<input type="hidden" name="amount" value="${generalList[g].productSpecs.psPrice }" />
									</c:if>
				...
				<br>
					...
					<br>
			<div align="right">
		    	 <button type="button" class="btn btn-info">共${page.rowCount}条数据</button>
		    	 <button type="button" class="btn btn-info">第${page.pageNum}页</button>
		    	 <button type="button" class="btn btn-info">共${page.pageCount}页</button>
		    	 <button type="button" class="btn btn-info" onclick="paging('1')">首页</button>
		    	 <button type="button" class="btn btn-info" onclick="paging('${page.pageNum - 1}')">上一页</button>
		    	 <button type="button" class="btn btn-info" onclick="paging(${page.pageNum + 1})">下一页</button>
		    	 <button type="button" class="btn btn-info" onclick="paging('${page.pageCount}')">尾页</button>
			</div>
			<h4 class="w3ls-title">
